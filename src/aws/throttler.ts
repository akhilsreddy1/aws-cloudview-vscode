 ...
 */
/**
 * Invoked by the scheduler when an SDK call surfaces an expired-credentials
 * error shape (ExpiredToken, ExpiredTokenException, InvalidClientTokenId,
 * etc.). The handler is expected to clear cached AWS sessions / SDK clients
 * so the *next* call re-resolves credentials from disk. It is the wiring
 * layer's responsibility to debounce — the scheduler may invoke this many
 * times during a refresh storm.
 */
export type AuthErrorHandler = (info: { service: string; operation: string; error: unknown }) => void;
...
export class AwsRequestScheduler {
  ...
  /**
   * Register a callback to be invoked whenever an SDK call's error matches an
   * expired-credentials shape. Pass `undefined` to clear. Only one handler is
   * stored (the wiring layer is the single owner of cache invalidation).
   */
  public setAuthErrorHandler(handler: AuthErrorHandler | undefined): void {
  ...
  public async run<T>(service: string, operation: string, task: () => Promise<T>): Promise<T> {
    ...
    while (true) {
      ...
      try {
        ...
        // Not retryable. If it looks like expired credentials, notify the
        // wiring layer so it can clear cached AWS sessions / clients. We
        // still propagate the original error — the *next* call will get
        // fresh credentials. The handler debounces its own work.
        if (this.isAuthExpiredError(error)) {
          try {
            this.onAuthError?.({ service, operation, error });
          } catch (handlerErr) {
            this.logger.warn(`onAuthError handler threw: ${this.describeError(handlerErr)}`);
          }
        }
        throw error;
      } finally {
  ...
  /**
   * Heuristic for "the AWS credentials we used for this call are expired."
   * Matches both the canonical SDK error names and the looser textual shapes
   * that show up from the credential chain or STS. Conservative on purpose —
   * a false positive only causes one extra cache clear on the next reset.
   */
  private isAuthExpiredError(error: unknown): boolean {
    if (error == null || (typeof error !== "object" && !(error instanceof Error))) return false;
    const err = error as Error & { name?: string; Code?: string; code?: string };
    const codeBag = [err.name, err.Code, err.code]
      .filter((x): x is string => typeof x === "string")
      .map((x) => x.toLowerCase());
    const codeMatches = (s: string): boolean =>
      s === "expiredtoken" ||
      s === "expiredtokenexception" ||
      s === "invalidclienttokenid" ||
      s === "tokenrefreshrequired" ||
      s === "credentialsexpired";
    if (codeBag.some(codeMatches)) return true;

    const msg = (error instanceof Error ? error.message : String(error)).toLowerCase();
    // Two-word match keeps this from flagging unrelated "expired" messages.
    return msg.includes("expired") && (msg.includes("token") || msg.includes("credential") || msg.includes("security"));
  }

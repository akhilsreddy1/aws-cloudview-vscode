...
export class AwsClientFactory {
  ...
  public async athena(scope: AwsScope): Promise<AthenaClient> {
    return this.resolveAndCreate(scope, "athena", AthenaClient);
  }

  /** Returns a cached API Gateway v1 (REST APIs) client for the given scope. */
  public async apiGateway(scope: AwsScope): Promise<APIGatewayClient> {
    return this.resolveAndCreate(scope, "apigateway", APIGatewayClient);
  }

  /** Returns a cached API Gateway v2 (HTTP + WebSocket APIs) client for the given scope. */
  public async apiGatewayV2(scope: AwsScope): Promise<ApiGatewayV2Client> {
    return this.resolveAndCreate(scope, "apigatewayv2", ApiGatewayV2Client);
  }

  /** Returns a cached Glue client for the given scope. Used to list/run ETL jobs. */
  public async glue(scope: AwsScope): Promise<GlueClient> {
    return this.resolveAndCreate(scope, "glue", GlueClient);
  }

  /** Returns a cached Secrets Manager client for the given scope. Used to list, view, and update secrets. */
  public async secretsManager(scope: AwsScope): Promise<SecretsManagerClient> {
    return this.resolveAndCreate(scope, "secretsmanager", SecretsManagerClient);
  }

  /** Returns a cached Systems Manager (SSM) client for the given scope. Used for parameters, documents, and run/automation executions. */
  public async ssm(scope: AwsScope): Promise<SSMClient> {
    return this.resolveAndCreate(scope, "ssm", SSMClient);
  }

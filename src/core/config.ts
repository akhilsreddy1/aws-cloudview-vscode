...
export function readCloudViewConfiguration(): CloudViewConfiguration {
  ...
  return {
    ...
    refreshTimeoutSeconds: config.get<number>("refresh.timeoutSeconds", 0),
    awsConfigFilePath: (config.get<string>("aws.configFilePath", "") || "").trim(),
    awsCredentialsFilePath: (config.get<string>("aws.credentialsFilePath", "") || "").trim(),
    storagePath: (config.get<string>("storage.path", "") || "").trim()
  };
}

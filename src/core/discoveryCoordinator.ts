...
export interface RefreshOptions {
  ...
  /**
   * Optional pre-resolved profile sessions so the UI can STS-resolve once per
   * refresh and reuse the outcome (credential failure summary + scoped discovery).
   * When omitted, {@link DiscoveryCoordinator.refreshSelectedProfiles} resolves
   * via {@link CloudViewPlatform.sessionManager}.
   */
  resolvedSessions?: readonly AwsProfileSession[];
  /**
   * Fired once per `(profile × region × service)` scope batch after individual
   * resource-type discovers settle. Consumers use this for refresh summaries.
   */
  onScopeOutcome?: (outcome: RefreshScopeOutcome) => void;
}

/** One failing resource-type discover within a `(scope, service)` refresh batch. */
export interface RefreshScopeResourceFailure {
  resourceType: string;
  message: string;
}

export interface RefreshScopeOutcome {
  readonly scope: AwsScope;
  /** Service id (`ec2`, `lambda`, …). */
  readonly service: string;
  readonly successDefinitionCount: number;
  readonly failures: readonly RefreshScopeResourceFailure[];
}
...
export class DiscoveryCoordinator {
  ...
  public async refreshServiceScope(scope: AwsScope, service: string, options: RefreshOptions = {}): Promise<ResourceNode[]> {
    ...
    const failures: RefreshScopeResourceFailure[] = [];
    let successDefinitionCount = 0;

    settled.forEach((outcome, index) => {
      const def = definitions[index];
      if (outcome.status === "fulfilled") {
        successDefinitionCount += 1;
        combined.push(...outcome.value);
      } else {
        const message = outcome.reason instanceof Error ? outcome.reason.message : String(outcome.reason);
        failures.push({ resourceType: def.type, message });
    ...
    options.onScopeOutcome?.({
      scope,
      service,
      successDefinitionCount,
      failures,
    });
  ...
  public async refreshSelectedProfiles(options: RefreshOptions = {}): Promise<void> {
    const profiles =
      options.resolvedSessions !== undefined
        ? [...options.resolvedSessions]
        : await this.platform.sessionManager.getSelectedProfileSessions();

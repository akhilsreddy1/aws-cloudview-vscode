...
/**
 * REST API ARN — Account-aware identifier for v1 (`apigateway`) APIs.
 * AWS itself uses two ARN shapes for these (the management plane uses
 * `arn:aws:apigateway:<region>::/restapis/<id>` with an empty account; the
 * execute-api plane uses `arn:aws:execute-api:<region>:<account>:<api>/...`).
 * We pick the management-plane form for cache identity since it's stable
 * regardless of stage, and synthesize the account into a custom path so
 * our generic per-account scoping logic still keys correctly.
 */
export function buildApiGatewayRestApiArn(region: string, accountId: string, apiId: string): string {
  return `arn:aws:apigateway:${region}:${accountId}:/restapis/${apiId}`;
}

/** Stage of a REST API. AWS treats stages as deployments of a parent restapi. */
export function buildApiGatewayStageArn(region: string, accountId: string, apiId: string, stageName: string): string {
  return `arn:aws:apigateway:${region}:${accountId}:/restapis/${apiId}/stages/${stageName}`;
}

/** v2 API ARN — covers HTTP + WebSocket APIs (the v2 service unifies them). */
export function buildApiGatewayV2ApiArn(region: string, accountId: string, apiId: string): string {
  return `arn:aws:apigateway:${region}:${accountId}:/apis/${apiId}`;
}

/** Stage of a v2 API. */
export function buildApiGatewayV2StageArn(region: string, accountId: string, apiId: string, stageName: string): string {
  return `arn:aws:apigateway:${region}:${accountId}:/apis/${apiId}/stages/${stageName}`;
}

/** Glue ETL job ARN. */
export function buildGlueJobArn(region: string, accountId: string, jobName: string): string {
  return `arn:aws:glue:${region}:${accountId}:job/${jobName}`;
}

/** Glue Data Catalog crawler ARN. */
export function buildGlueCrawlerArn(region: string, accountId: string, crawlerName: string): string {
  return `arn:aws:glue:${region}:${accountId}:crawler/${crawlerName}`;
}

/** Glue workflow ARN. */
export function buildGlueWorkflowArn(region: string, accountId: string, workflowName: string): string {
  return `arn:aws:glue:${region}:${accountId}:workflow/${workflowName}`;
}

/** Glue trigger ARN. */
export function buildGlueTriggerArn(region: string, accountId: string, triggerName: string): string {
  return `arn:aws:glue:${region}:${accountId}:trigger/${triggerName}`;
}

/**
 * Secrets Manager secret ARN. Note: real secret ARNs from the API carry a
 * 6-char random suffix (`-AbCdEf`); prefer the ARN returned by `ListSecrets`.
 * This builder is a fallback for when only the name is known.
 */
export function buildSecretsManagerSecretArn(region: string, accountId: string, secretName: string): string {

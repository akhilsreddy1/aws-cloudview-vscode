...
export interface CloudViewConfiguration {
  ...
  /**
   * Optional explicit path to the AWS config file. Empty string means "use
   * AWS_CONFIG_FILE env var, else default `~/.aws/config`". Useful on
   * Windows / corp setups where the home dir VS Code sees doesn't match
   * where the user actually keeps their AWS configuration.
   */
  awsConfigFilePath: string;
  /**
   * Optional explicit path to the AWS credentials file. Empty string means
   * "use AWS_SHARED_CREDENTIALS_FILE env var, else default `~/.aws/credentials`".
   */
  awsCredentialsFilePath: string;
  /**
   * Optional override for the directory holding the local SQLite cache.
   * Empty string means "use VS Code's `globalStorageUri`". When non-empty,
   * the path is expanded (`~` → home dir) and `mkdir -p`'d at startup.
   * Useful on corp setups where the default location is on a read-only or
   * anti-virus-monitored path.
   */
  storagePath: string;
}

...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    ...
    // When proxy-related settings change, drop cached AWS clients so the next
    // call rebuilds them with the new HTTP handler. Avoids forcing a full
    // extension restart just to pick up a corp-proxy change.
    ...
    try {
      await vscode.window.withProgress(
        ...
        async (_progress, token) => {
          // Call the discovery coordinator to refresh the resources for the selected profiles and regions and services.
          await platform.discoveryCoordinator.refreshSelectedProfiles({
            force: true,  // Bypass the TTL gate and force a fresh API call.
            cancellation: token,
          });

...
import { DiscoveryCoordinator, type RefreshScopeOutcome } from "../../core/discoveryCoordinator";
...
describe("DiscoveryCoordinator", () => {
  ...
  it("refreshServiceScope tolerates partial failures (allSettled)", async () => {
    ...
    const outcomes: RefreshScopeOutcome[] = [];
    ...
    const result = await coordinator.refreshServiceScope(makeScope(), "ec2", {
      onScopeOutcome: (o) => outcomes.push(o),
    });

    expect(goodDef.discoverer.discover).toHaveBeenCalled();
    expect(badDef.discoverer.discover).toHaveBeenCalled();
    expect(outcomes).toHaveLength(1);
    expect(outcomes[0].successDefinitionCount).toBe(1);
    expect(outcomes[0].failures).toHaveLength(1);
    expect(outcomes[0].failures[0].resourceType).toBe("AWS::EC2::VPC");
    expect(outcomes[0].failures[0].message).toContain("boom");
    expect(result).toHaveLength(1);

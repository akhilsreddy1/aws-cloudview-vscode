...
export function registerDefaultActions(actionRegistry: ActionRegistry): void {
  ...
  // CloudFormation — live-tail stack events while a deploy is in flight.
  actionRegistry.register({
    id: "cloudView.cfn.watchStackEvents",
    title: "\u{1F4E1} Watch Stack Events (live)",
    order: 0,
    isAvailable: (resource) => resource.type === ResourceTypes.cfnStack,
    execute: async (resource) => {
      await vscode.commands.executeCommand("cloudView.cfn.watchStackEvents", resource.arn);
    }
  });

  // CloudFormation — show export / import / nested-stack dependencies.
  actionRegistry.register({
    id: "cloudView.cfn.viewDependencies",
    title: "🔗 View Stack Dependencies",
    order: 0,
    isAvailable: (resource) => resource.type === ResourceTypes.cfnStack,
    execute: async (resource) => {
      await vscode.commands.executeCommand("cloudView.cfn.viewDependencies", resource.arn);
    }
  });

  // EC2 — open an SSM Session Manager interactive shell on a running instance.
  actionRegistry.register({
    id: "cloudView.ec2.startSession",
    title: "🔌 Start SSM Session (terminal)",
    order: 0,
    isAvailable: (resource) => {
    ...
    execute: async (resource) => {
      await vscode.commands.executeCommand("cloudView.ec2.startSession", resource.arn);
    }
  });

  // ECS — exec into a running task's container via `aws ecs execute-command`.
  actionRegistry.register({
    id: "cloudView.ecs.execCommand",
    title: "🔌 Exec into Container (terminal)",
    order: 0,
    isAvailable: (resource) => {
      if (resource.type !== ResourceTypes.ecsTask) return false;
      const status = String(resource.rawJson.lastStatus ?? "").toUpperCase();
      return status === "RUNNING";
    },
    execute: async (resource) => {
      await vscode.commands.executeCommand("cloudView.ecs.execCommand", resource.arn);
    }
  });

  // API Gateway — drill into the per-API routes / integrations table.
  // Applies to both REST (v1) and HTTP/WebSocket (v2) APIs; stages have
  // their own URL action via Open in Console.
  actionRegistry.register({
    id: "cloudView.apigateway.viewRoutes",
    title: "\u{1F517} View Routes & Integrations",
    order: 0,
    isAvailable: (resource) =>
      resource.type === ResourceTypes.apiGatewayRestApi ||
      resource.type === ResourceTypes.apiGatewayV2Api,
    execute: async (resource) => {
      await vscode.commands.executeCommand("cloudView.apigateway.viewRoutes", resource.arn);
    }
  });

  // Load balancer — drill into listeners → target groups → targets with
  // live health states.
  actionRegistry.register({
    id: "cloudView.elbv2.viewHierarchy",
    title: "\u{1F500} View Hierarchy (listeners → targets)",
    order: 0,
    isAvailable: (resource) => resource.type === ResourceTypes.alb,
    execute: async (resource) => {
      await vscode.commands.executeCommand("cloudView.elbv2.viewHierarchy", resource.arn);
    }
  });

  // Glue ETL job — open the runs panel to trigger/stop runs, watch status
  // live, and jump to per-run CloudWatch logs.
  actionRegistry.register({
    id: "cloudView.glue.viewJobRuns",
    title: "▶ View Runs & Logs",
    order: 0,
    isAvailable: (resource) => resource.type === ResourceTypes.glueJob,
    execute: async (resource) => {
      await vscode.commands.executeCommand("cloudView.glue.viewJobRuns", resource.arn);
    }
  });

  // Secrets Manager — open the value panel to view (masked) and save a new
  // version of the secret value.
  actionRegistry.register({
    id: "cloudView.secretsmanager.viewSecret",
    title: "🔐 View / Edit Value",
    order: 0,
    isAvailable: (resource) => resource.type === ResourceTypes.secretsManagerSecret,
    execute: async (resource) => {
      await vscode.commands.executeCommand("cloudView.secretsmanager.viewSecret", resource.arn);
    }
  });

  // RDS — drill into Cluster → instances → connectivity, plus read-replica
  // chains, with live instance status.
  actionRegistry.register({
    id: "cloudView.rds.viewHierarchy",
    title: "🔀 View Hierarchy (cluster → instances → replicas)",
    order: 0,
    isAvailable: (resource) =>
      resource.type === ResourceTypes.rdsCluster || resource.type === ResourceTypes.rdsInstance,
    execute: async (resource) => {
      await vscode.commands.executeCommand("cloudView.rds.viewHierarchy", resource.arn);
    }
  });

  // ECS — drill into Cluster → services → tasks → containers with live-ish
  // status from the last discovery.
  actionRegistry.register({
    id: "cloudView.ecs.viewHierarchy",
    title: "🔀 View Hierarchy (cluster → services → tasks)",
    order: 0,
    isAvailable: (resource) =>
      resource.type === ResourceTypes.ecsCluster || resource.type === ResourceTypes.ecsService,
    execute: async (resource) => {
      await vscode.commands.executeCommand("cloudView.ecs.viewHierarchy", resource.arn);
    }
  });

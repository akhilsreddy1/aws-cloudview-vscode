...
import { LogsInsightsPanel } from "./logsInsightsPanel";
    ...
    this.panel.webview.onDidReceiveMessage(async (msg) => {
      try {
        if (msg.type === "refresh") {
          ...
          // Hand off to the Insights query panel pre-scoped to this group's
          // account + region. Skips the profile/region quick-picks and
          // pre-ticks the log group so the user can just write a query.
          await LogsInsightsPanel.open(this.platform, {
            scope: { accountId: this.resource.accountId, region: this.resource.region },
            prefilledLogGroup: this.logGroupName,
          });
        }
      } catch (err) {
        this.postError(err);
...
.logs-mode-tabs { background: var(--surface); border-bottom: 1px solid var(--border); padding: 0 24px; display: flex; gap: 2px; flex-shrink: 0; align-items: center; }
.logs-mode-tabs .logs-mode-spacer { flex: 1; }
.logs-mode-tabs .logs-insights-btn {
  background: transparent; border: 1px solid #6d28d9; color: #6d28d9;
  padding: 4px 12px; border-radius: 14px; font-size: 12px; font-weight: 600;
  cursor: pointer; margin: 6px 0; transition: background .15s;
  display: inline-flex; align-items: center; gap: 6px;
}
.logs-mode-tabs .logs-insights-btn:hover { background: #ede9fe; }
.logs-mode-tab {
...
<div class="logs-mode-tabs">
  ...
  <span class="logs-mode-spacer"></span>
  <button class="logs-insights-btn" id="logs-insights-btn" title="Open this group in the CloudWatch Logs Insights query panel">\u26A1 Run Insights Query</button>
</div>
...
// ── Insights launcher ────────────────────────────────────────────────────
// Opens the CWLI query panel pre-scoped to this account/region with this
// log group already in the picker. Lives in the tabs row (not a tab itself)
// because it opens a separate webview rather than switching in-panel modes.
const insightsBtn = document.getElementById('logs-insights-btn');
if (insightsBtn) {
  insightsBtn.addEventListener('click', () => {
    vscode.postMessage({ type: 'openInsights' });
  });
}

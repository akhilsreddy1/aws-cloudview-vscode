          ...
          <div class="steps">
            <div class="step">
              ...
              <div class="step-body">
                ...
                <div class="step-desc">Pick one or more AWS profiles from the sidebar — they're used to discover resources. Static keys, IAM <code>role_arn</code> + <code>source_profile</code>, SSO, and web-identity profiles all resolve through the standard SDK chain. If a profile fails (expired SSO, invalid keys, missing role trust), CloudView shows the specific reason and a Reload Profiles action.</div>
              </div>
            </div>
            <div class="step">
              <div class="step-num">2</div>
              <div class="step-body">
                <div class="step-title">Custom AWS config paths (optional)</div>
                <div class="step-desc">If your <code>~/.aws/config</code> or <code>~/.aws/credentials</code> live somewhere non-standard (Windows roaming profile, network share, corp setup), set <code>cloudView.aws.configFilePath</code> and <code>cloudView.aws.credentialsFilePath</code> in settings. Resolution order is setting → <code>AWS_CONFIG_FILE</code> / <code>AWS_SHARED_CREDENTIALS_FILE</code> env vars → defaults. Path changes are picked up live; just click Reload AWS Profiles after.</div>
              </div>
            </div>
            <div class="step">
            ...
            <div class="step">
              ...
              <div class="step-body">
                <div class="step-title">Custom storage location (optional)</div>
                <div class="step-desc">CloudView caches discovered resources in a local SQLite file under VS Code's globalStorage by default. On corporate machines where that path is read-only, AV-quarantined, or on a roaming profile, set <code>cloudView.storage.path</code> to a writable directory (e.g. <code>~/Documents/cloudview-cache</code>). The path is probed at startup; if it's not writable, CloudView logs a warning and falls back to the default. Restart the extension after changing.</div>
              </div>
            </div>
            <div class="step">

...
export class DynamoDbItemsPanel {
      ...
      try {
        if (msg.type === "scan") {
          ...
          await this.downloadCsv(
            msg.csv,
            typeof msg.suggestedName === "string" ? msg.suggestedName : "dynamodb-items.csv",
          );
        }
      } catch (err: unknown) {
   ...
   * built in the webview from the already-fetched items (no extra DynamoDB
   * round-trip). Defaults the save dialog to the first workspace folder, or
  ...
  private async downloadCsv(csv: string, suggestedName: string): Promise<void> {
    ...
    try {
      ...
      const choice = await vscode.window.showInformationMessage(
        `Saved DynamoDB items to ${target.fsPath}.`,
        open,
      );
      if (choice === open) {
...
<head>
  ...
  <style>
    ...
    .results-toolbar {
      display: none; align-items: center; gap: 16px; padding: 8px 12px; margin: 0 0 12px;
      background: var(--surface-2); border: 1px solid var(--border); border-radius: var(--radius-sm);
      font-size: 11px; color: var(--muted);
      position: sticky; top: 0; z-index: 2;
    }
    .results-toolbar.has-rows { display: flex; }
    .results-toolbar .row-summary { flex: 1; display: flex; gap: 16px; flex-wrap: wrap; }
    .results-toolbar .row-summary strong { color: var(--text-2); font-weight: 600; }
    .btn-csv { background: transparent; border: 1px solid var(--border-2); color: var(--text); padding: 4px 10px; border-radius: var(--radius-sm); font-size: 11px; font-weight: 600; cursor: pointer; white-space: nowrap; }
...
<body>
  ...
  <div class="content">
    <div class="pane active" id="pane-items">
      <div id="results-toolbar" class="results-toolbar">
        <div id="summary" class="row-summary"></div>
        <button class="btn-csv" id="download-csv-btn" title="Download these items as CSV (one row per item, columns = union of attributes)">⬇ Download CSV</button>
      </div>
  ...
  <script nonce="${n}">
    ...
    // the items in CSV without re-scanning DynamoDB.
    var latestItems = [];
    var latestMode = '';
    ...
    function renderItems(items, mode, scannedCount, consumedCapacity) {
      ...
      if (latestItems.length === 0) {
        ...
        container.innerHTML = '';
        toolbar.classList.remove('has-rows');
        return;
      }
      empty.style.display = 'none';
      var bits = ['<strong>' + latestItems.length + '</strong> item' + (latestItems.length === 1 ? '' : 's')];
      ...
      toolbar.classList.add('has-rows');

      var html = '';
      for (var i = 0; i < latestItems.length; i++) {
        var it = latestItems[i];
        var pretty;
    ...
    function buildCsv() {
      // DynamoDB items are schemaless — compute the union of attribute names
      // across the returned items, ordered by first-appearance. Each row is
      // serialised against that union; missing attributes become empty cells.
      var seen = Object.create(null);
      var columns = [];
      for (var i = 0; i < latestItems.length; i++) {
        var item = latestItems[i].item || {};
        for (var k in item) {
          if (Object.prototype.hasOwnProperty.call(item, k) && !seen[k]) {
            seen[k] = true;
            columns.push(k);
          }
        }
      ...
      for (var j = 0; j < latestItems.length; j++) {
        var it = latestItems[j].item || {};
        lines.push(columns.map(function(c) { return csvField(it[c]); }).join(','));
      }
      return lines.join('\\r\\n') + '\\r\\n';
    }
    document.getElementById('download-csv-btn').onclick = function() {
      ...
      var modeSuffix = latestMode ? ('-' + latestMode) : '';
      vscode.postMessage({
        type: 'downloadCsv',
        csv: buildCsv(),
        suggestedName: 'dynamodb-${escapeHtml(this.tableName)}' + modeSuffix + '-' + stamp + '.csv',
      });
    };

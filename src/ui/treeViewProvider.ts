...
const EXTERNAL_SERVICES: ExternalServiceEntry[] = [
  ...
  {
    serviceId: "ssm",
    serviceLabel: "Systems Manager",
    command: "cloudView.ssm.open",
  },
];
...
export class ServiceTreeProvider implements vscode.TreeDataProvider<ServiceTreeNode> {
  ...
  public getChildren(): ServiceTreeNode[] {
    ...
    // Synthetic (external) entries are positioned by their serviceId within
    // SERVICE_DISPLAY_ORDER, just like real services — so e.g. Systems Manager
    // can sit after Step Functions instead of always being pinned to the top.
    const externalById = new Map(EXTERNAL_SERVICES.map((e) => [e.serviceId, e]));
    ...
    // Any external entry not positioned in the display order falls back to the
    // top, preserving the previous "synthetic services first" behaviour.
    for (const ext of EXTERNAL_SERVICES) {
      if (!emittedExternal.has(ext.serviceId)) {
        ordered.unshift({
          kind: "service",
          serviceId: ext.serviceId,
          serviceLabel: ext.serviceLabel,
    ...
    // Remaining registered services not listed in the display order.

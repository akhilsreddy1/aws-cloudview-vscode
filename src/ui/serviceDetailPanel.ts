      ...
      try {
        await vscode.window.withProgress(
          ...
          async (progress) => {
            ...
            const totalScopes = this.accountIds.length * this.queryRegions.length;
            let done = 0;
            for (const accountId of this.accountIds) {
              for (const region of this.queryRegions) {
                ...
                // Surface the current account+region in the toast so users
                // running multi-account/region refreshes can see whose data is
                // being fetched right now.
                progress.report({
                  message: `[${done}/${totalScopes}] acct ${accountId} · ${region}`,
                });
...
<head>
  ...
  <style>${BASE_STYLES}
    ...
    #msk-topics-toolbar { margin-top: 8px; margin-bottom: 8px; }
    #msk-topic-filter { width: 100%; box-sizing: border-box; padding: 6px 8px; font-size: 12px;
      border: 1px solid var(--border); border-radius: var(--radius-sm); background: var(--surface); color: inherit; font-family: inherit; }
    #msk-topic-detail { margin-top: 14px; font-size: 11px; padding-top: 8px; border-top: 1px solid var(--border); }
    #msk-topic-detail .msk-topic-meta { margin: 0 0 6px 0; line-height: 1.45; color: var(--muted); font-size: 11px; }
    #msk-topic-detail pre.msk-configs { margin: 4px 0 0; padding: 8px; max-height: 200px; overflow: auto; background: var(--surface-2); border: 1px solid var(--border); border-radius: var(--radius-sm); font-family: ui-monospace, monospace; font-size: 10px; line-height: 1.35; white-space: pre-wrap; word-break: break-word; }
    #msk-topic-detail .msk-partitions { margin-top: 8px; max-height: 240px; overflow: auto; }
...
<body>
  ...
  <script nonce="${nonce}">
    ...
    /** MSK topics drawer: last loaded cluster ARN and rows (for filtering + DescribeTopic calls). */
    var MSK_TOPICS_CLUSTER_ARN = null;
    var MSK_TOPICS_CACHE = [];

    function bindMskTopicFilterOnce() {
      var filterEl = document.getElementById('msk-topic-filter');
      if (!filterEl || filterEl.dataset.mskBound) return;
      filterEl.dataset.mskBound = '1';
      filterEl.oninput = function() { renderMskTopicsTable(); };
    }

    function renderMskTopicsTable() {
      var listEl = document.getElementById('msk-topics-list');
      if (!listEl) return;
      var filterEl = document.getElementById('msk-topic-filter');
      var fq = '';
      try {
        fq = filterEl ? String(filterEl.value || '').toLowerCase().trim() : '';
      } catch (e) { fq = ''; }
      var topics = MSK_TOPICS_CACHE || [];
      var filtered = !fq ? topics : topics.filter(function(t) {
        var n = String(t.topicName || '').toLowerCase();
        return n.indexOf(fq) !== -1;
      });
      if (topics.length === 0 && !MSK_TOPICS_CLUSTER_ARN) {
        listEl.innerHTML = '';
        return;
      }
      if (topics.length === 0) {
        listEl.innerHTML = '<p style="font-size:12px;color:var(--muted);">No topics in cache — load topics above.</p>';
        return;
      }
      var thead = '<thead><tr><th>Topic</th><th>Partitions</th><th>Replication</th><th style="white-space:nowrap;">Details</th></tr></thead>';
      var body = '<tbody>' + filtered.map(function(t) {
        var tnAttr = encodeURIComponent(t.topicName || '');
        var parts = '<tr><td>' + escHtml(t.topicName || '') + '</td><td>' + escHtml(String(t.partitionCount != null ? t.partitionCount : "")) +
          '</td><td>' + escHtml(String(t.replicationFactor != null ? t.replicationFactor : "")) + '</td><td>' +
          '<button type="button" class="cv-btn" style="padding:2px 8px;font-size:10px;margin:0;" data-msk-topic-name="' + tnAttr + '">' +
          'Describe</button></td></tr>';
        return parts;
      }).join('') + '</tbody>';
      var emptyFiltered = fq && filtered.length === 0 ?
        '<p style="font-size:11px;color:var(--muted);margin-bottom:8px;">No topics match \\u201C' + escHtml(fq) + '\\u201D.</p>' : '';
      listEl.innerHTML = emptyFiltered +
        '<table style="width:100%;font-size:11px;border-collapse:collapse;">' + thead + body + '</table>';
      bindMskDescribeButtons(listEl);
    }

    function bindMskDescribeButtons(container) {
      if (!container) return;
      container.querySelectorAll('button[data-msk-topic-name]').forEach(function(btn) {
        btn.onclick = function(ev) {
          ev.stopPropagation();
          var tn = '';
          try { tn = decodeURIComponent(btn.getAttribute('data-msk-topic-name') || ''); } catch (e3) {}
          if (!MSK_TOPICS_CLUSTER_ARN || !tn) return;
          var detailEl = document.getElementById('msk-topic-detail');
          if (detailEl) detailEl.innerHTML = '<p class="msk-topic-meta">Loading <strong>' + escHtml(tn) + '\\u2026</strong></p>';
          vscode.postMessage({ type: 'describeMskTopic', clusterArn: MSK_TOPICS_CLUSTER_ARN, topicName: tn });
        };
      });
    }
    ...
    function getFilteredByTab(tabId) {
      ...
      return ALL_RESOURCES.filter(function(r) {
        ...
        switch (tabId) {
          ...
          case 'rds_databases': return rt === 'aws.rds.cluster' || rt === 'aws.rds.instance';
          // MSK
    ...
    function renderColumnCell(r, col) {
      ...
      var COLUMN_BUTTONS = [
        ...
        { key: '__cfnEvents', service: 'cloudformation', cls: 'cv-invoke-btn', attr: 'data-cfn-events-arn', label: '\\u{1F4E1} Watch Events', msgType: 'cfnWatchEvents', arnKey: 'cfnEventsArn' },
        { key: '__cfnDependencies', service: 'cloudformation', cls: 'cv-invoke-btn', attr: 'data-cfn-deps-arn', label: '\\u{1F517} Dependencies', msgType: 'cfnViewDependencies', arnKey: 'cfnDepsArn' },
        { key: '__apigatewayRoutes', service: 'apigateway', cls: 'cv-invoke-btn', attr: 'data-ag-routes-arn', label: '\\u{1F517} View Routes', msgType: 'apigatewayViewRoutes', arnKey: 'agRoutesArn', resourceType: ['aws.apigateway.rest-api', 'aws.apigatewayv2.api'] },
        { key: '__lbHierarchy', service: 'ec2', cls: 'cv-invoke-btn', attr: 'data-lb-hierarchy-arn', label: '\\u{1F500} Hierarchy', msgType: 'lbViewHierarchy', arnKey: 'lbHierarchyArn', resourceType: 'aws.elbv2.load-balancer' },
        { key: '__glueRuns', service: 'glue', cls: 'cv-invoke-btn', attr: 'data-glue-runs-arn', label: '\\u25B6 Runs & Logs', msgType: 'glueViewRuns', arnKey: 'glueRunsArn' },
        { key: '__secretView', service: 'secretsmanager', cls: 'cv-invoke-btn', attr: 'data-secret-view-arn', label: '\\u{1F510} View / Edit', msgType: 'secretViewValue', arnKey: 'secretViewArn' },
        { key: '__rdsHierarchy', service: 'rds', cls: 'cv-invoke-btn', attr: 'data-rds-hierarchy-arn', label: '\\u{1F500} Hierarchy', msgType: 'rdsViewHierarchy', arnKey: 'rdsHierarchyArn', resourceType: ['aws.rds.cluster', 'aws.rds.instance'] },
        { key: '__ecsHierarchy', service: 'ecs', cls: 'cv-invoke-btn', attr: 'data-ecs-hierarchy-arn', label: '\\u{1F500} Hierarchy', msgType: 'ecsViewHierarchy', arnKey: 'ecsHierarchyArn', resourceType: ['aws.ecs.cluster', 'aws.ecs.service'] },
        // Glue crawler: toggle Start / Stop based on current State. Renderer
        // below picks the label.
        { key: '__glueCrawlerToggle', service: 'glue', cls: 'cv-invoke-btn', attr: 'data-glue-crawler-arn', label: 'Toggle crawler', msgType: 'glueCrawlerToggle', arnKey: 'glueCrawlerArn', resourceType: 'aws.glue.crawler', special: 'glueCrawler' },
        { key: '__glueWorkflowRun', service: 'glue', cls: 'cv-invoke-btn', attr: 'data-glue-workflow-arn', label: '\\u25B6 Run workflow', msgType: 'glueWorkflowRun', arnKey: 'glueWorkflowArn', resourceType: 'aws.glue.workflow' },
        // Glue trigger: toggle Activate / Deactivate based on current State.
        { key: '__glueTriggerToggle', service: 'glue', cls: 'cv-invoke-btn', attr: 'data-glue-trigger-arn', label: 'Toggle trigger', msgType: 'glueTriggerToggle', arnKey: 'glueTriggerArn', resourceType: 'aws.glue.trigger', special: 'glueTrigger' },
        // EC2 SSM session — only meaningful on running instances. Hidden otherwise.
        { key: '__ec2Session', service: 'ec2', cls: 'cv-invoke-btn', attr: 'data-ec2-session-arn', label: '\\u{1F50C} SSM Session', msgType: 'ec2StartSession', arnKey: 'ec2SessionArn', resourceType: 'aws.ec2.instance', special: 'ec2Session' },
        // ECS exec — only meaningful on RUNNING tasks. Hidden otherwise.
        { key: '__ecsExec', service: 'ecs', cls: 'cv-invoke-btn', attr: 'data-ecs-exec-arn', label: '\\u{1F41A} Exec', msgType: 'ecsExecCommand', arnKey: 'ecsExecArn', resourceType: 'aws.ecs.task', special: 'ecsExec' },
      ];
      for (var i = 0; i < COLUMN_BUTTONS.length; i++) {
        var b = COLUMN_BUTTONS[i];
        if (col.key === b.key && SERVICE_ID === b.service) {
          // resourceType filter: string for single-type, array for multi-type
          // (e.g. API Gateway has both REST and V2 API types; the Routes
          // button applies to both but NOT to stage rows).
          if (b.resourceType && r.resourceType) {
            var allowed = Array.isArray(b.resourceType) ? b.resourceType : [b.resourceType];
            if (allowed.indexOf(r.resourceType) === -1) return '';
          }
          // Glue crawler: Start when READY/STOPPED, Stop when RUNNING/STOPPING.
          if (b.special === 'glueCrawler') {
            var gcState = String(getVal(r, 'State') || '').toUpperCase();
            var gcRunning = gcState === 'RUNNING' || gcState === 'STOPPING';
            var gcLabel = gcRunning ? '⏹ Stop' : '▶ Start';
            var gcCls = gcRunning ? 'cv-danger-btn' : b.cls;
            return '<button type="button" class="' + gcCls + '" ' + b.attr + '="' + escHtml(r.arn || '') + '" data-glue-action="' + (gcRunning ? 'stop' : 'start') + '">' + gcLabel + '</button>';
          }
          // EC2 SSM session: only render when the instance is running.
          if (b.special === 'ec2Session') {
            var ec2State = String(getVal(r, 'State.Name') || '').toLowerCase();
            if (ec2State !== 'running') return '';
            return '<button type="button" class="' + b.cls + '" ' + b.attr + '="' + escHtml(r.arn || '') + '">' + b.label + '</button>';
          }
          // ECS exec: only render when the task is RUNNING.
          if (b.special === 'ecsExec') {
            var ecsState = String(getVal(r, 'lastStatus') || getVal(r, 'status') || '').toUpperCase();
            if (ecsState !== 'RUNNING') return '';
            return '<button type="button" class="' + b.cls + '" ' + b.attr + '="' + escHtml(r.arn || '') + '">' + b.label + '</button>';
          }
          // Glue trigger: Activate when DEACTIVATED/CREATED, Deactivate when ACTIVATED/ACTIVATING.
          if (b.special === 'glueTrigger') {
            var gtState = String(getVal(r, 'State') || '').toUpperCase();
            var gtActivated = gtState === 'ACTIVATED' || gtState === 'ACTIVATING';
            var gtLabel = gtActivated ? '⏸ Deactivate' : '▶ Activate';
            var gtCls = gtActivated ? 'cv-danger-btn' : b.cls;
            return '<button type="button" class="' + gtCls + '" ' + b.attr + '="' + escHtml(r.arn || '') + '" data-glue-action="' + (gtActivated ? 'stop' : 'start') + '">' + gtLabel + '</button>';
          }
          return '<button type="button" class="' + b.cls + '" ' + b.attr + '="' + escHtml(r.arn || '') + '">' + b.label + '</button>';
        }
      }
    ...
    function renderTable() {
      ...
      if (col && !(SERVICE_ID === 'vpc' && activeTab === 'vpc_hierarchy')) {
      ...
      var BTN_BINDINGS = [
        ...
        { attr: 'data-cfn-events-arn', msgType: 'cfnWatchEvents', arnKey: 'cfnEventsArn' },
        { attr: 'data-cfn-deps-arn', msgType: 'cfnViewDependencies', arnKey: 'cfnDepsArn' },
        { attr: 'data-ec2-session-arn', msgType: 'ec2StartSession', arnKey: 'ec2SessionArn' },
        { attr: 'data-ecs-exec-arn', msgType: 'ecsExecCommand', arnKey: 'ecsExecArn' },
        { attr: 'data-ag-routes-arn', msgType: 'apigatewayViewRoutes', arnKey: 'agRoutesArn' },
        { attr: 'data-lb-hierarchy-arn', msgType: 'lbViewHierarchy', arnKey: 'lbHierarchyArn' },
        { attr: 'data-glue-runs-arn', msgType: 'glueViewRuns', arnKey: 'glueRunsArn' },
        { attr: 'data-secret-view-arn', msgType: 'secretViewValue', arnKey: 'secretViewArn' },
        { attr: 'data-rds-hierarchy-arn', msgType: 'rdsViewHierarchy', arnKey: 'rdsHierarchyArn' },
        { attr: 'data-ecs-hierarchy-arn', msgType: 'ecsViewHierarchy', arnKey: 'ecsHierarchyArn' },
        { attr: 'data-glue-crawler-arn', msgType: 'glueCrawlerToggle', arnKey: 'glueCrawlerArn', extra: 'glueAction' },
        { attr: 'data-glue-workflow-arn', msgType: 'glueWorkflowRun', arnKey: 'glueWorkflowArn' },
        { attr: 'data-glue-trigger-arn', msgType: 'glueTriggerToggle', arnKey: 'glueTriggerArn', extra: 'glueAction' },
      ];
      BTN_BINDINGS.forEach(function(binding) {
        tbody.querySelectorAll('button[' + binding.attr + ']').forEach(function(btn) {
          btn.onclick = function(ev) {
            ev.stopPropagation();
            var payload = { type: binding.msgType, arn: btn.dataset[binding.arnKey] };
            // Some buttons carry an extra field (e.g. Glue toggles forward
            // their data-glue-action which is "start" or "stop"). Pull it
            // off the dataset under the binding-declared key.
            if (binding.extra) {
              payload[binding.extra] = btn.dataset[binding.extra];
            }
    ...
    function openDrawer(resource) {
      ...
      if (IS_MSK && resource.resourceType === 'aws.msk.cluster') {
        ...
        html += '<p style="font-size:11px;color:var(--muted);margin-bottom:8px;line-height:1.4;">Uses MSK Kafka control-plane APIs (<code>ListTopics</code>, <code>DescribeTopic</code>, <code>DescribeTopicPartitions</code>). IAM must allow those actions on the cluster. MSK Serverless may not expose the same APIs as provisioned clusters &mdash; error messages explain common cases.</p>';
        ...
        html += '<div id="msk-topics-toolbar" style="display:none;">';
        html += '<label for="msk-topic-filter" style="font-size:10px;color:var(--muted);display:block;margin-bottom:4px;">Filter by topic name</label>';
        html += '<input type="text" id="msk-topic-filter" autocomplete="off" spellcheck="false" placeholder="Substring match \\u2026" />';
        html += '</div>';
        html += '<div id="msk-topics-list"></div>';
        html += '<div id="msk-topic-detail"></div></div>';
      }
      ...
      if (loadTopicsBtn && IS_MSK && resource.resourceType === 'aws.msk.cluster') {
        loadTopicsBtn.onclick = function() {
          MSK_TOPICS_CLUSTER_ARN = resource.arn || null;
          MSK_TOPICS_CACHE = [];
          var toolbar = document.getElementById('msk-topics-toolbar');
          var detailEl = document.getElementById('msk-topic-detail');
          var listEl = document.getElementById('msk-topics-list');
          if (toolbar) toolbar.style.display = 'none';
          if (detailEl) detailEl.innerHTML = '';
          if (listEl) listEl.innerHTML = '<p style="font-size:11px;color:var(--muted);">Loading topics\\u2026</p>';
          var filterEl = document.getElementById('msk-topic-filter');
          if (filterEl) {
            delete filterEl.dataset.mskBound;
            filterEl.value = '';
            bindMskTopicFilterOnce();
          }
    ...
    window.addEventListener('message', function(e) {
      ...
      if (msg.type === 'mskTopicsResult') {
        ...
        MSK_TOPICS_CLUSTER_ARN = msg.clusterArn || MSK_TOPICS_CLUSTER_ARN;
        var toolbar = document.getElementById('msk-topics-toolbar');
        var detailElClear = document.getElementById('msk-topic-detail');
        if (detailElClear) detailElClear.innerHTML = '';
        var listEl = document.getElementById('msk-topics-list');
        if (!listEl) return;
        if (msg.error) {
          MSK_TOPICS_CACHE = [];
          if (toolbar) toolbar.style.display = 'none';
          listEl.innerHTML = '<p style="color:#b91c1c;font-size:12px;line-height:1.5;white-space:pre-wrap;">' + escHtml(String(msg.error)) + '</p>';
          return;
        }
        MSK_TOPICS_CACHE = msg.topics || [];
        if (toolbar) toolbar.style.display = MSK_TOPICS_CACHE.length ? 'block' : 'none';
        bindMskTopicFilterOnce();
        renderMskTopicsTable();
      }
      if (msg.type === 'mskTopicDescribeResult') {
        var wrap = document.getElementById('msk-topic-detail');
        if (!wrap) return;
        if (msg.error) {
          wrap.innerHTML = '<p style="color:#b91c1c;font-size:12px;line-height:1.5;white-space:pre-wrap;"><strong>' + escHtml(msg.topicName || '') + '</strong>: ' + escHtml(String(msg.error)) + '</p>';
          return;
        }
        var d = msg.describe || {};
        var statusLine = '';
        statusLine += d.topicArn ? '<div style="margin:2px 0;font-size:10px;color:var(--muted);word-break:break-all;">Arn: <code>' + escHtml(String(d.topicArn)) + '</code></div>' : '';
        statusLine += '<div class="msk-topic-meta">Status: <strong>' + escHtml(String(d.status != null ? d.status : '\\u2014')) + '</strong>' +
          ' \\u00B7 Partitions (describe): <strong>' + escHtml(String(d.partitionCount != null ? d.partitionCount : '\\u2014')) + '</strong>' +
          ' \\u00B7 Replication factor: <strong>' + escHtml(String(d.replicationFactor != null ? d.replicationFactor : '\\u2014')) + '</strong>';
        statusLine += d.configsNote ? ' \\u00B7 <em>' + escHtml(String(d.configsNote)) + '</em>' : '';
        statusLine += '</div>';
        var cfg = String(d.configsUtf8 != null ? d.configsUtf8 : '');
        var prettyCfg = '';
        try {
          if (cfg.trim().length > 0 && cfg.trim().charAt(0) === '{') {
            prettyCfg = '<div style="margin-top:8px;"><div style="font-size:11px;color:var(--muted);">Topic configs (JSON)</div>' +
              '<pre class="msk-configs">' + escHtml(JSON.stringify(JSON.parse(cfg), null, 2)) + '</pre></div>';
          }
        } catch (je) {}
        var rawCfgHtml = '';
        if (cfg.length > 0) {
          rawCfgHtml =
            ...
            (prettyCfg ? 'Topic configs (raw UTF-8)' : 'Topic configs (decoded UTF-8)') +
            '</div><pre class="msk-configs">' + escHtml(cfg) + '</pre></div>';
        }
        var partRows = (msg.partitions || []).slice();
        var pHead = '<thead><tr><th>Partition</th><th>Leader</th><th>Replicas</th><th>ISR</th></tr></thead>';
        var pBody = '<tbody>' + partRows.map(function(p) {
          var rep = (p.replicas && p.replicas.length) ? escHtml(String(p.replicas.join(', '))) : '';
          var isr = (p.isr && p.isr.length) ? escHtml(String(p.isr.join(', '))) : '';
          return '<tr><td>' + escHtml(String(p.partition != null ? p.partition : '')) + '</td><td>' +
            escHtml(String(p.leader != null ? p.leader : '')) + '</td><td>' + rep + '</td><td>' + isr + '</td></tr>';
        }).join('') + '</tbody>';
        wrap.innerHTML = '<div class="cv-detail-section-title" style="margin-bottom:8px;font-size:12px;">Topic: ' +
          escHtml(msg.topicName || '') + '</div>' + statusLine + prettyCfg + rawCfgHtml +
          '<div class="msk-topic-meta" style="margin-top:10px;"><strong>' + escHtml(String(partRows.length)) + '</strong> partition row(s)</div>' +
          '<table class="msk-partitions" style="width:100%;border-collapse:collapse;font-size:10px;"><caption style="caption-side:top;text-align:left;padding:6px 0 4px;font-size:11px;color:var(--muted);">Partitions (DescribeTopicPartitions)</caption>' +
          pHead + pBody + '</table>';
      }
    });

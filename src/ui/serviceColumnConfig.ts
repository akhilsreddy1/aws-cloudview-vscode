...
const ec2Config: ServiceViewConfig = {
  ...
  columns: [
    ...
    { key: "__ec2Session", label: "Session", type: "text" },
    ...
    { key: "__lbHierarchy", label: "Hierarchy", type: "text" },
  ...
  tabs: [
    { id: "ec2_instances", label: "Instances", filter: (r) => r.type === "aws.ec2.instance",
      columns: ["name", "id", "__ec2Session", "__ec2StartStop", "InstanceType", "State.Name", "AgeDays", "EbsOptimized", "CpuCredits", "PrivateIpAddress", "PublicIpAddress", "VpcId", "LaunchTime"] },
    { id: "running", label: "Running", filter: (r) => r.type === "aws.ec2.instance" && String(rawVal(r, "State.Name") ?? "").toLowerCase() === "running",
      columns: ["name", "id", "__ec2Session", "__ec2StartStop", "InstanceType", "AgeDays", "EbsOptimized", "CpuCredits", "PrivateIpAddress", "PublicIpAddress", "VpcId", "LaunchTime"] },
    { id: "stopped", label: "Stopped", filter: (r) => r.type === "aws.ec2.instance" && String(rawVal(r, "State.Name") ?? "").toLowerCase() === "stopped",
      columns: ["name", "id", "__ec2StartStop", "InstanceType", "AgeDays", "PrivateIpAddress", "VpcId", "LaunchTime"] },
    { id: "ec2_lbs", label: "Load Balancers", filter: (r) => r.type === "aws.elbv2.load-balancer",
      columns: ["name", "__lbHierarchy", "Type", "Scheme", "State.Code", "DNSName", "IpAddressType", "ListenerCount", "ListenerSummary", "AvailabilityZoneList", "VpcId", "CreatedTime"] },
...
const rdsConfig: ServiceViewConfig = {
  ...
  columns: [
    ...
    { key: "type", label: "Type", type: "text" },
    { key: "__rdsHierarchy", label: "Hierarchy", type: "text" },
...
const ecsConfig: ServiceViewConfig = {
  ...
  columns: [
    { key: "name", label: "Name", type: "text" },
    { key: "__ecsHierarchy", label: "Hierarchy", type: "text" },
    { key: "__ecsExec", label: "Exec", type: "text" },
  ...
  tabs: [
    {
      ...
      columns: ["name", "__ecsHierarchy", "id", "type", "status", "runningTasksCount", "activeServicesCount"],
    },
    {
      id: "ecs_services",
      label: "Services",
      filter: (r) => r.type === "aws.ecs.service",
      columns: ["name", "__ecsHierarchy", "__ecsScale", "id", "type", "status", "desiredCount", "runningCount", "launchType"],
    },
    {
      id: "ecs_tasks",
      label: "Tasks",
...
const cloudformationConfig: ServiceViewConfig = {
  ...
  columns: [
    ...
    { key: "__cfnEvents", label: "Events", type: "text" },
    { key: "__cfnDependencies", label: "Dependencies", type: "text" },
...
// ─── API Gateway (REST + HTTP + WebSocket under one UI service) ─────────────

const apigatewayConfig: ServiceViewConfig = {
  ...
  columns: [
    { key: "name", label: "Name", type: "text" },
    { key: "Description", label: "Description", type: "text" },
    { key: "ProtocolType", label: "Protocol", type: "status" },
    { key: "__apigatewayRoutes", label: "Routes", type: "text" },
    // Stage-only columns (whitelisted per tab so they don't show on the APIs tab)
    { key: "ApiId", label: "API ID", type: "code" },
    { key: "InvokeUrl", label: "Invoke URL", type: "code" },
    { key: "CreatedDate", label: "Created", type: "date" },
  ],
  stats: [
    { label: "Total APIs", color: "#7c3aed", compute: (r) => r.filter((x) => x.type === ResourceTypes.apiGatewayRestApi || x.type === ResourceTypes.apiGatewayV2Api).length },
    { label: "REST", color: "#1d4ed8", compute: (r) => r.filter((x) => x.type === ResourceTypes.apiGatewayRestApi).length },
    { label: "HTTP", color: "#15803d", compute: (r) => r.filter((x) => x.type === ResourceTypes.apiGatewayV2Api && String(rawVal(x, "ProtocolType") ?? "").toUpperCase() === "HTTP").length },
    { label: "WebSocket", color: "#c2410c", compute: (r) => r.filter((x) => x.type === ResourceTypes.apiGatewayV2Api && String(rawVal(x, "ProtocolType") ?? "").toUpperCase() === "WEBSOCKET").length },
...
const glueConfig: ServiceViewConfig = {
  serviceId: "glue",
  ...
  columns: [
    ...
    { key: "name", label: "Name", type: "text" },
    // Job columns
    { key: "__glueRuns", label: "Runs", type: "text" },
    { key: "JobType", label: "Type", type: "status" },
    { key: "LastRunState", label: "Last Run", type: "status" },
    { key: "GlueVersion", label: "Glue Version", type: "text" },
    { key: "WorkerType", label: "Worker Type", type: "text" },
    { key: "NumberOfWorkers", label: "Workers", type: "number" },
    { key: "LastRunStarted", label: "Last Started", type: "date" },
    { key: "Role", label: "IAM Role", type: "code" },
    // Crawler columns
    { key: "__glueCrawlerToggle", label: "Action", type: "text" },
    { key: "State", label: "State", type: "status" },
    { key: "DatabaseName", label: "Database", type: "text" },
    { key: "TargetSummary", label: "Targets", type: "text" },
    { key: "Schedule", label: "Schedule", type: "code" },
    { key: "LastCrawlStatus", label: "Last Crawl", type: "status" },
    { key: "LastCrawlStarted", label: "Last Crawl Started", type: "date" },
    // Workflow columns
    { key: "__glueWorkflowRun", label: "Run", type: "text" },
    { key: "Description", label: "Description", type: "text" },
    { key: "LastRunStatus", label: "Last Run", type: "status" },
    { key: "MaxConcurrentRuns", label: "Max Concurrent", type: "number" },
    // Trigger columns
    { key: "__glueTriggerToggle", label: "Action", type: "text" },
    { key: "TriggerType", label: "Type", type: "status" },
    { key: "WorkflowName", label: "Workflow", type: "code" },
    { key: "FirstActionTarget", label: "Target", type: "code" },
    { key: "ActionCount", label: "Targets", type: "number" },
  ],
  stats: [
    { label: "Jobs", color: "#c925d1", compute: (r) => r.filter((x) => x.type === ResourceTypes.glueJob).length },
    { label: "Crawlers", color: "#1d4ed8", compute: (r) => r.filter((x) => x.type === ResourceTypes.glueCrawler).length },
    { label: "Workflows", color: "#15803d", compute: (r) => r.filter((x) => x.type === ResourceTypes.glueWorkflow).length },
    { label: "Triggers", color: "#854d0e", compute: (r) => r.filter((x) => x.type === ResourceTypes.glueTrigger).length },
    { label: "Crawlers running", color: "#d97706", compute: (r) => r.filter((x) => x.type === ResourceTypes.glueCrawler && String(rawVal(x, "State") ?? "") === "RUNNING").length },
    { label: "Triggers activated", color: "#166534", compute: (r) => r.filter((x) => x.type === ResourceTypes.glueTrigger && String(rawVal(x, "State") ?? "") === "ACTIVATED").length },
  ],
  tabs: [
    {
      id: "glue_jobs",
      label: "ETL Jobs",
      filter: (r) => r.type === ResourceTypes.glueJob,
      columns: ["name", "__glueRuns", "JobType", "LastRunState", "GlueVersion", "WorkerType", "NumberOfWorkers", "LastRunStarted", "Role"],
    },
    {
      id: "glue_crawlers",
      label: "Crawlers",
      filter: (r) => r.type === ResourceTypes.glueCrawler,
      columns: ["name", "__glueCrawlerToggle", "State", "DatabaseName", "TargetSummary", "Schedule", "LastCrawlStatus", "LastCrawlStarted", "Role"],
    },
    {
      id: "glue_workflows",
      label: "Workflows",
      filter: (r) => r.type === ResourceTypes.glueWorkflow,
      columns: ["name", "__glueWorkflowRun", "Description", "LastRunStatus", "LastRunStarted", "MaxConcurrentRuns"],
    },
    {
      id: "glue_triggers",
      label: "Triggers",
      filter: (r) => r.type === ResourceTypes.glueTrigger,
      columns: ["name", "__glueTriggerToggle", "TriggerType", "State", "WorkflowName", "FirstActionTarget", "ActionCount", "Schedule"],
    },
    {
      id: "glue_failed",
      label: "Failed last run",
      filter: (r) =>
        (r.type === ResourceTypes.glueJob && ["FAILED", "ERROR", "TIMEOUT"].includes(String(rawVal(r, "LastRunState") ?? ""))) ||
        (r.type === ResourceTypes.glueCrawler && String(rawVal(r, "LastCrawlStatus") ?? "") === "FAILED") ||
        (r.type === ResourceTypes.glueWorkflow && String(rawVal(r, "LastRunStatus") ?? "") === "ERROR"),
    },
  ],
};
...
const secretsManagerConfig: ServiceViewConfig = {
  serviceId: "secretsmanager",
  serviceLabel: "Secrets Manager",
  iconKey: "secretsmanager",
  columns: [
    { key: "name", label: "Secret Name", type: "text" },
    { key: "__secretView", label: "Value", type: "text" },
    { key: "Description", label: "Description", type: "text" },
    { key: "RotationEnabled", label: "Rotation", type: "bool" },
    { key: "LastChangedDate", label: "Last Changed", type: "date" },
    { key: "LastAccessedDate", label: "Last Accessed", type: "date" },
    { key: "KmsKeyId", label: "KMS Key", type: "code" },
    { key: "OwningService", label: "Managed By", type: "text" },
  ],
  stats: [
    { label: "Total Secrets", color: "#DD3344", compute: (r) => r.length },
    { label: "Rotation enabled", color: "#166534", compute: (r) => r.filter((x) => rawVal(x, "RotationEnabled") === true).length },
    { label: "Service-managed", color: "#854d0e", compute: (r) => r.filter((x) => Boolean(rawVal(x, "OwningService"))).length },
  ],
  tabs: [
    { id: "all", label: "All Secrets" },
    { id: "sm_rotation", label: "Rotation on", filter: (r) => rawVal(r, "RotationEnabled") === true },
    { id: "sm_user", label: "User-managed", filter: (r) => !rawVal(r, "OwningService") },
  ],
};
...
export function resolveResourceValue(resource: ResourceNode, key: string): unknown {
  if (
    ...
    key === "__cfnTemplate" ||
    key === "__cfnEvents" ||
    key === "__cfnDependencies" ||
    key === "__ec2StartStop" ||
    key === "__rdsStartStop" ||
    key === "__apigatewayRoutes" ||
    key === "__lbHierarchy" ||
    key === "__glueRuns" ||
    key === "__glueCrawlerToggle" ||
    key === "__glueWorkflowRun" ||
    key === "__glueTriggerToggle" ||
    key === "__secretView" ||
    key === "__rdsHierarchy" ||
    key === "__ecsHierarchy" ||
    key === "__ec2Session" ||
